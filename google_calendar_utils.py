...
class GoogleCalendarUtils:
    ...
    async def get_auth_url(self, user_id: str) -> str | None:
        ...
        # Depuração: Logar os valores exatos que estão sendo usados para client_config
    ...
    async def handle_oauth2_callback(self, authorization_response_url: str) -> dict:
        ...
        # Depuração: Logar a URL de resposta exata e os parâmetros
...
Você está me enviando o `google_calendar_utils.py` novamente.

Os logs anteriores que você me mostrou indicavam que o erro `InsecureTransportError` acontecia na linha de `fetch_token`, que é onde a biblioteca `oauthlib` estava fazendo a validação da URL.

Você está 100% certo em apontar para esse erro e é uma armadilha muito comum. O `InsecureTransportError` ocorre quando a biblioteca OAuthlib (usada pelo `google-auth-oauthlib`) encontra uma URL que deveria ser HTTPS, mas é HTTP.

No seu `google_calendar_utils.py`, na classe `GoogleCalendarUtils`, as URLs `self.redirect_uri` e `self.frontend_url` são populadas por `os.getenv()`. O problema, como discutimos, está em garantir que os valores dessas variáveis de ambiente no ambiente de execução do Cloud Run realmente comecem com `https://`.

Se você me confirmou que:
*   `GOOGLE_REDIRECT_URI` no Cloud Run é `https://eixa-760851989407.us-east1.run.app/oauth2callback`
*   `FRONTEND_URL` no Cloud Run é `https://eixa.web.app`
*   E esses mesmos valores estão nos "Authorized redirect URIs" e "Authorized JavaScript origins" no seu Google Cloud Console.

Então, teoricamente, o `InsecureTransportError` não deveria mais ocorrer.

**Se o erro persiste com essas configurações, as únicas possibilidades restantes (extremamente raras, mas que já vi acontecer):**

1.  **Cache Teimoso do Cloud Run:** O serviço não está pegando a versão mais recente das variáveis de ambiente ou dos arquivos. A única maneira de combater isso é um `gcloud run deploy` com `--no-cache` ou, em casos extremos, deletar o serviço e recriá-lo (anotando todas as configurações antes).
2.  **Proxy ou Load Balancer:** Se houver alguma camada de proxy ou load balancer entre o Google e seu Cloud Run que esteja de alguma forma rebaixando o protocolo de HTTPS para HTTP antes de chegar ao seu contêiner. Isso é muito, muito improvável em um setup padrão de Cloud Run, pois ele gerencia HTTPS automaticamente.
3.  **Erro no `requirements.txt`:** Se `requests-oauthlib` ou `oauthlib` não estiverem na versão correta ou estiverem com alguma inconsistência de dependência. Eu sugeri fixar as versões mais recentes.

**Por favor, confirme se você já adicionou `requests-oauthlib` ao seu `requirements.txt` e fez o deployment.** Esta é a peça que faltava para a biblioteca `google-auth-oauthlib` ter todas as suas dependências satisfeitas.

**Se mesmo após adicionar `requests-oauthlib` e um novo deployment o erro persistir, o próximo passo seria depurar o `authorization_response_url` que chega ao seu backend, para ver o protocolo real que ele contém.**

**Adicionei uma linha de DEBUG LOG no `handle_oauth2_callback` do `google_calendar_utils.py` para imprimir a URL exata que está sendo processada. Isso nos dirá se o problema está na URL que chega ou na interpretação dela pela biblioteca.**

---

**`google_calendar_utils.py` (Com log de depuração adicional):**
