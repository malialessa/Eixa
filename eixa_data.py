...
async def get_daily_tasks_data(user_id: str, date_str: str) -> dict:
    ...
    logger.debug(f"EIXA_DATA | get_daily_tasks_data: Getting daily tasks for user '{user_id}' on date '{date_str}'. Doc path: {doc_ref.path}") # Novo log
    ...
    data = doc.to_dict()
    logger.debug(f"EIXA_DATA | get_daily_tasks_data: Raw data fetched for daily tasks for '{user_id}' on '{date_str}': {data}") # Novo log
...
async def save_daily_tasks_data(user_id: str, date_str: str, data: dict):
    doc_ref = get_task_doc_ref(user_id, date_str)
    logger.debug(f"EIXA_DATA | save_daily_tasks_data: Attempting to save daily tasks for user '{user_id}' on '{date_str}'. Doc path: {doc_ref.path}. Data: {data}") # Novo log
    try:
        await asyncio.to_thread(doc_ref.set, data)
        logger.info(f"EIXA_DATA | save_daily_tasks_data: Daily tasks for user '{user_id}' on '{date_str}' saved to Firestore successfully.") # Adicionado para clareza no log
...
async def get_all_daily_tasks(user_id: str) -> dict:
    ...
    logger.debug(f"EIXA_DATA | get_all_daily_tasks: Attempting to retrieve all daily tasks for user '{user_id}' from collection ID: {agenda_ref.id}. Full path: {agenda_ref.parent.id}/{agenda_ref.id}") # Novo log
    all_tasks = {}
    try:
        docs = await asyncio.to_thread(lambda: list(agenda_ref.stream()))
        if not docs:
            logger.info(f"EIXA_DATA | get_all_daily_tasks: No daily task documents found for user '{user_id}'.") # Adicionado para clareza no log
        for doc_snapshot in docs:
            date_str = doc_snapshot.id
            all_tasks[date_str] = await get_daily_tasks_data(user_id, date_str)
            logger.debug(f"EIXA_DATA | get_all_daily_tasks: Retrieved doc '{date_str}' for user '{user_id}'.") # Novo log
        logger.info(f"EIXA_DATA | get_all_daily_tasks: Retrieved all daily tasks for user '{user_id}'. Total days: {len(all_tasks)}") # Adicionado para clareza no log
    except Exception as e:
        logger.error(f"EIXA_DATA | get_all_daily_tasks: Error retrieving all daily tasks for user '{user_id}': {e}", exc_info=True) # Adicionado para clareza no log
    return all_tasks
...
async def get_project_data(user_id: str, project_id: str) -> dict:
    ...
    logger.debug(f"EIXA_DATA | get_project_data: Getting project '{project_id}' for user '{user_id}'. Doc path: {doc_ref.path}") # Novo log
    doc = await asyncio.to_thread(doc_ref.get)
    if not doc.exists:
        logger.info(f"EIXA_DATA | get_project_data: Project '{project_id}' not found for user '{user_id}'. Returning empty dict.") # Adicionado para clareza no log
        return {}
    data = doc.to_dict()
    logger.debug(f"EIXA_DATA | get_project_data: Raw data fetched for project '{project_id}' for '{user_id}': {data}") # Novo log
    return data

async def save_project_data(user_id: str, project_id: str, data: dict):
    doc_ref = get_project_doc_ref(user_id, project_id)
    logger.debug(f"EIXA_DATA | save_project_data: Attempting to save project '{project_id}' for user '{user_id}'. Doc path: {doc_ref.path}. Data: {data}") # Novo log
    try:
        await asyncio.to_thread(doc_ref.set, data)
        logger.info(f"EIXA_DATA | save_project_data: Project '{project_id}' for user '{user_id}' saved to Firestore successfully.") # Adicionado para clareza no log
    except Exception as e:
...
async def get_all_projects(user_id: str) -> list[dict]:
    ...
    try:
        ...
        if not docs:
            logger.info(f"EIXA_DATA | get_all_projects: No project documents found for user '{user_id}'.") # Adicionado para clareza no log
        for doc in docs:
            ...
            logger.debug(f"EIXA_DATA | get_all_projects: Retrieved project '{doc.id}' for user '{user_id}'.") # Novo log

        logger.info(f"EIXA_DATA | get_all_projects: Retrieved all projects for user '{user_id}'. Total projects: {len(all_projects)}") # Adicionado para clareza no log
    except Exception as e:
        logger.error(f"EIXA_DATA | get_all_projects: Error retrieving all projects for user '{user_id}': {e}", exc_info=True) # Adicionado para clareza no log
    return all_projects
...
async def get_user_history(user_id: str, interactions_collection_logical_name: str = EIXA_INTERACTIONS_COLLECTION, limit: int = 10) -> list[dict]:
    try:
        ...
        logger.debug(f"EIXA_DATA | get_user_history: Querying history for user '{user_id}' from collection '{interactions_ref.id}'. Limit: {limit}") # Novo log
        ...
        logger.info(f"EIXA_DATA | get_user_history: Retrieved {len(history)} interaction history items for user '{user_id}'.") # Adicionado para clareza no log
        return history
    except Exception as e:
